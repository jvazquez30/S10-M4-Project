...
export default function Quotes() {
  const { data: quotes, isLoading: quotesLoading, isFetching: fetchingQuotes } = useGetQuotesQuery()
  const [toggleFake, { isLoading: toggleLoad}] = useToggleFakeMutation()
  const [deleteQuote, { isLoading: deleteLoad}] = useDeleteQuoteMutation()
  ...
  return (
    <div id="quotes">
      <h3>
        Quotes
        {deleteLoad && ' deletion processing...'} 
        {toggleLoad && ' toggling...'}
        {(quotesLoading || fetchingQuotes) && ' loading...'}
        </h3>
      <div>
        {
          quotes?.filter(qt => {
